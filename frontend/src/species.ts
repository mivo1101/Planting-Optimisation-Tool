...
const searchInput = document.getElementById('speciesSearch') as HTMLInputElement;
const searchBtn = document.getElementById('speciesSearchBtn') as HTMLButtonElement;
...
function openModal(item: Species) {
  ...
  const modalContent = modal!.querySelector('.modal-content-body') as HTMLElement;
...
async function handleSearch() {
  ...
  renderGrid(results);
  searchBtn.innerHTML = '<span class="search-icon">🔍</span><span>Search</span>';
}
...
function init() {
  ...
  emptyMsg.hidden = false;
  emptyMsg.textContent = "Enter keywords like '2000mm' or 'boundary systems' to find species.";
}
